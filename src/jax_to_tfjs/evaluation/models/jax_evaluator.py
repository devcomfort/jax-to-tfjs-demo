...
def evaluate_jax_model(
    checkpoint, test_images, test_labels, with_probs=False
) -> Union[
    ...
    반환:
        ...
        with_probs가 True인 경우:
            tuple: (metrics, predictions, probabilities) - 메트릭 객체, 예측, 확률 값
    """
    # 체크포인트에서 params 추출
    if isinstance(checkpoint, dict):
        ...
        if "params" in checkpoint:
            ...
            and "params" in checkpoint["model"]
        ):
    ...
    for i in tqdm(range(num_batches), desc="JAX 모델 평가 중"):
        ...
        try:
            ...
            logging.error(
                f"체크포인트 키: {list(params.keys()) if isinstance(params, dict) else 'Not a dict'}"
            )
            raise
    ...
    if with_probs:
        ...
        # JAX 배열을 numpy 배열로 변환
        np_test_labels = np.array(test_labels_jax)
